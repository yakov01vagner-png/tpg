...
describe('Б5: бюджет', () => {
  ...
  it('сутки, час, сетка и сейв укладываются в телефон', { retry: 2 }, () => {
    ...
    // 8.4–8.9 мс. Столько же она стоила и на закрытии 0.8 (проверено прогоном
    // этого же файла на коммите 91a20c1), так что граница здесь — про шум
    // файла, а не про бюджет.
    expect(dayMs).toBeLessThan(10)
