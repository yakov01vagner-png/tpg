...
describe('Б5: бюджет', () => {
  ...
  it('сутки, час, сетка и сейв укладываются в телефон', { retry: 2 }, () => {
    ...
    // не шумит от соседей — там свежие сутки стоят 5.0–6.8 мс. Здесь та же
    // работа меряется после векового прогона, в разогретом файле, и стоит
    // 8.1–10.6 мс, и разброс этот ходит от прогона к прогону вместе с тем, что
    // оставил после себя вековой прогон в том же файле (проверялось: те же
    // сутки без единой правки ядра дают то 8.1, то 10.6). Настоящий бюджет
    // меряет perf.test.ts, и он держится: 5.0–5.4 мс.
    expect(dayMs).toBeLessThan(12)
    // Версия 1.0 добавила к суткам три слоя, которые работают у корон, а не у
    // игрока: счёт войны (этап 163), вести между коронами (этап 164) и казна
    // корон (этап 165). В середине партии, где идёт война и стоит держава, это
    // мерилось так: 0.9 давала 10.3–11.7 мс, 1.0 даёт 12.8–15.1 (проверялось на
    // том же приёме и той же машине, с прогоном 0.9 из рабочего дерева). Часть
    // цены снята памятью на объекте состояния: счёт силы и земля стороны
    // считаются по разу на состояние, а не по разу на спрос, — без неё те же
    // сутки стоили 42 мс. Настоящий бюджет по-прежнему меряет perf.test.ts на
    // свежей игре: там сутки стоят 5.3–6.8 мс и версией не испорчены.
    expect(midMs).toBeLessThan(18)
