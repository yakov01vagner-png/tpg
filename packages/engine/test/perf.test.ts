...
describe('бюджет на телефон', () => {
  ...
  it('такт мира: сутки за пять миллисекунд, час — за одну', { retry: 2 }, () => {
    let current = game
    const day = ms(() => {
    ...
    // Шесть на материке (этап 48): поселений четыре сотни против двух с
    // небольшим, и такт вырос было почти вдвое; шаг к цели дружины и
    // множители снабжения теперь помнятся на мир, и сутки снова в бюджете.
