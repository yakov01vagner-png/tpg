...
export const MIGRATIONS: Readonly<Record<number, Migration>> = {
  ...
  /**
   * v26 → v27: отряд из людей (этап 166).
   *
   * Книга набора начинается пустой, и это верно: сейв 0.9 не помнит, где и
   * когда набраны его копейщики. Первый же такт сведёт её с числами — люди
   * старого отряда окажутся набранными там, где он стоит, и в тот день, когда
   * сейв открыли. Иначе пришлось бы выдумывать им прошлое.
   */
  26: (data) => ({
    ...data,
    roll: data.roll ?? [],
  ...
  /**
   * v33 → v34: клеймо помнит год и починки (этап 179).
   *
   * Поля необязательные и лежат внутри вещи, а не в корне состояния, — трогать
   * тут нечего: вещь без года просто не скажет, какого она года, и это честно.
